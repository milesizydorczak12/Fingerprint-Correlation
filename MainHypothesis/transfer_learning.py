...
def run_deep_learning(BATCH_SIZE=16, LEARNING_RATE=0.01, NUM_EPOCHS=25, LR_DECAY_FACTOR=0.8, LR_DECAY_STEP=2, MOMENTUM=0.9, WEIGHT_DECAY=5e-5, IS_PRETRAINED=True, CUDA_DEVICE_IDS=[0]):
    def train_model(model, criterion, optimizer, scheduler, dataloaders, class_names, num_epochs=25):
        since = time.time()
        ...
        with open('results/resnet_exclude_' + get_finger_left_out(class_names) + '.txt', 'w') as fout:
            fout.write('Identifying FGRP {}'.format(get_finger_left_out(class_names)))
    ...
    print('leave out:', get_finger_left_out(class_names))
    ...
    finger_left_out = get_finger_left_out(class_names)
    torch.save(model_ft.state_dict(), 'models/resnet_fingerprint' + get_finger_left_out(class_names))
